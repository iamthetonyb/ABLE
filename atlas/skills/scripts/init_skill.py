#!/usr/bin/env python3
"""
ATLAS Skill Initializer — Creates a new skill from template.

Adapted from OpenClaw skill-creator (https://github.com/openclaw/openclaw).
Enhanced for ATLAS's multi-agent system with trust levels, malware scanning,
...
Examples:
    init_skill.py my-skill --path atlas/skills/library
    init_skill.py my-skill --path atlas/skills/library --resources scripts,references
    init_skill.py notion-sync --path atlas/skills/library --resources scripts --examples
"""
...
Part of ATLAS skill: {skill_name}
...
def init_skill(skill_name: str, path: str, resources: list, include_examples: bool):
  ...
  {skill_name}:
    ...
    use_count: 0
""")
    print("  5. Run malware scan: python -c \"from atlas.security.malware_scanner import scan_skill; import asyncio; asyncio.run(scan_skill('atlas/skills/library/{skill_name}'))\"")
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a new ATLAS skill directory with SKILL.md template.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s my-skill --path atlas/skills/library
  %(prog)s notion-sync --path atlas/skills/library --resources scripts,references
  %(prog)s pdf-analyzer --path atlas/skills/library --resources scripts,assets --examples
        """,
    )
    ...
    print(f"Initializing ATLAS skill: {skill_name}")
