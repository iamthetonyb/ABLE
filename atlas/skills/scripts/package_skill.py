#!/usr/bin/env python3
"""
ATLAS Skill Packager — Validates and packages a skill into a .skill file.

Adapted from OpenClaw skill-creator (https://github.com/openclaw/openclaw).
Enhanced for ATLAS with malware scanning before packaging.

Usage:
    package_skill.py <path/to/skill-folder> [output-directory]

Examples:
    package_skill.py atlas/skills/library/my-skill
    package_skill.py atlas/skills/library/my-skill ./dist
"""
...
def package_skill(skill_path_str: str, output_dir_str: str = None) -> Path | None:
    ...
    try:
        ...
        print(f"Or copy '{skill_filename.name}' to atlas/skills/library/ and unzip")
...
def main():
    if len(sys.argv) < 2:
        ...
        print()
        print("Examples:")
        print("  package_skill.py atlas/skills/library/my-skill")
        print("  package_skill.py atlas/skills/library/my-skill ./dist")
        sys.exit(1)

    skill_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else None

    print(f"Packaging ATLAS skill: {skill_path}")
    if output_dir:
